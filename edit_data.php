        ...
        <div class="col-md-8 col-xs-8" id="content-customer-body">
          <div class="box">
              <div class="box-header with-border" id="load-data">
                <h3 class="box-title">Edit Data Pengaduan</h3>
              </div>
              <!-- /.box-header -->
              <div class="box-body">
                   ...
                   <?php
                      Include('koneksi.php');
                      $id=$_GET['id'];
                      $sql=mysql_query("SELECT documents.doc_id, documents.complaint, disposition.name_disposition from documents INNER JOIN disposition ON documents.code_disposition = disposition.code_disposition where doc_id='$id'");
                      while($row=mysql_fetch_array($sql)){
                   ?>
                    <form action="proses_edit_data.php" method="post" enctype="multipart/form-data">
                      <input type="hidden" value="<?php echo $row['doc_id']?>" name="id"></input>

                      <div class="form-group">
                        <label>Isi Pengaduan:</label>
                        <textarea name="isi_pengaduan" class="form-control" rows="15"><?php echo $row['complaint']?></textarea>
                      </div>

                      <div class="form-group">
                        <label>Disposisi Sebelumnya:</label>
                        <input type="text" readonly class="form-control" rows="15" value="<?php echo $row['name_disposition']?>"></input>
                      </div>

                      <div class="form-group">
                        <label>Pilih Lagi Disposisi:</label>
                        <select class="form-control" id="sel1" name="kode_disposisi">
                            <?php
                            include("koneksi.php");
                            $tampil_nama_disposisi = mysql_query("SELECT * FROM disposition");
                            while($w=mysql_fetch_array($tampil_nama_disposisi))
                            {
                            ?>
                               <option value=<?php echo $w['code_disposition']; ?> selected><?php echo $w['name_disposition']; ?></option>
                            <?php 
                            } ?>
                        </select>
                      </div>
                      <button name="submit" type="submit" class="btn btn-primary" value="Edit">Update</button>&nbsp;
                       <button class="btn btn-danger" value="Kembali"><a style="color: white" href="dokumen.php">Back</a></button>
                    </form>
                    <?php }
                    ?>
